...
async function getGlobalDynamicFields(projectId, callback) {
    $.ajax({
        type: 'GET',
        async: true,
        url: apiUrl + `api/dynamic_fields/global/suppliers/${projectId}`,
...
window.addEventListener('load', async () => {
    ...
    const { hasAccessToGlobalDynamicFieldsHandling } = supplierStateManager.getSupplier();

    if (hasAccessToGlobalDynamicFieldsHandling) {
        const { projectId } = supplierStateManager.getSupplier()

        await getGlobalDynamicFields(projectId, displayGlobalDynamicFields);
    }
});
